...
function candidateBlock(options: ShellTemplateOptions): string {
  ...
  const allGlobs = [...extraCacheRoots, '$_C/plugins/cache/nord-local/nord-mem']
    .map((root) => `"${root}"/[0-9]*/`)
    .join(' ');
  ...
  lines.push(`printf '%s\\n' "$_C/plugins/marketplaces/nord-local/plugin";`);
...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  const cacheRoots = [
    ...
    '$_C/plugins/cache/nord-local/nord-mem',
  ].map(shTokenToNode);
  const marketplace = shTokenToNode('$_C/plugins/marketplaces/nord-local/plugin');
