...
function detectMissingMarketplaceMarker(): void {
  ...
  const marketplaceCandidates = [
    resolve(home, '.claude', 'plugins', 'marketplaces', 'nord-local'),
    resolve(home, '.config', 'claude', 'plugins', 'marketplaces', 'nord-local'),
  ];
  const present = marketplaceCandidates.some(p => p && existsSync(p));
  const cacheCandidates = [
    resolve(home, '.claude', 'plugins', 'cache', 'nord-local', 'nord-mem'),
    resolve(home, '.config', 'claude', 'plugins', 'cache', 'nord-local', 'nord-mem'),
  ];
