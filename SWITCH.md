...
- **(b) Add alongside, under a distinct name** — edit the fork's
  ...
  (e.g. `"nord-local"`) before adding it, so both marketplaces coexist
...
sed -i 's/"name": "thedotmack"/"name": "nord-local"/' .claude-plugin/marketplace.json
...
#    (~/.claude/plugins/cache/nord-local/nord-mem/<version>/) and
...
claude plugin disable nord-mem@nord-local
...
claude plugin uninstall nord-mem@nord-local
claude plugin marketplace remove nord-local
...
`~/.claude/plugins/cache/nord-local/nord-mem/13.24.1/` was verified to carry the
